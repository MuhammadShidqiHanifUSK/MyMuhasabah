<x-app-layout>
    <x-slot name="header">
        <h2 class="mm-page-title">✏️ Edit Muhasabah</h2>
        <a href="{{ route('muhasabah.show', $muhasabah) }}" class="mm-btn mm-btn-secondary">
            ← Kembali
        </a>
    </x-slot>
            ...
            <form action="{{ route('muhasabah.update', $muhasabah) }}" method="POST">
                ...
                <div style="margin-bottom:1.5rem;">
                    ...
                    <textarea id="content" name="content" rows="7"
                              ...
                              placeholder="Ceritakan harimu...">{{ old('content', $muhasabah->content) }}</textarea>
                    @error('content')
                        <p class="mm-error-msg">⚠️ {{ $message }}</p>
                    @enderror
                </div>
